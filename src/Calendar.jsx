...
const datesAreDifferent = (date1, date2) =>
	(date1 && !date2) || (!date1 && date2) || (date1 && date2 && date1.getTime() !== date2.getTime());
...
const getActiveStartDate = props => {
	const { activeStartDate, maxDate, maxDetail, minDate, minDetail, value, view } = props;

	const rangeType = getView(view, minDetail, maxDetail);
	const valueFrom = getDetailValueFrom(value, minDate, maxDate, maxDetail) || activeStartDate || new Date();
	...
	renderContent() {
		...
		switch (view) {
			case 'century': {
				...
				return <CenturyView onClick={mergeFunctions(clickAction, onClickDecade)} {...commonProps} />;
			}
			case 'decade': {
				const { onClickYear } = this.props;

				return <DecadeView onClick={mergeFunctions(clickAction, onClickYear)} {...commonProps} />;
			}
			case 'year': {
				const { formatMonth, onClickMonth } = this.props;

				return (
					<YearView formatMonth={formatMonth} onClick={mergeFunctions(clickAction, onClickMonth)} {...commonProps} />
				);
			}
			case 'month': {
				const {
